...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        bees = None
        index = 0
        while not bees and pos.is_hive == False:
            if self.min_range <= index <= self.max_range:
                bees = pos.bees
            pos = pos.entrance
            index += 1
        if bees == None:
...
class LongThrower(ThrowerAnt):
    ...
    # OVERRIDE CLASS ATTRIBUTES HERE
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUIBUTES HERE
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        if self.health - amount <= 0:
            inflicted += self.damage
        l = self.place.bees[:]
        for bee in l:
            bee.reduce_health(inflicted)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        """
        # BEGIN Problem 12
        if gamestate.queen_in == True:
            return
        else:
            gamestate.queen_in = True
        ...
        while place.exit:
            if place.ant:
                ...
                if place.ant.is_container and place.ant.ant_contained:
                    double(place.ant.ant_contained)
            place = place.exit
        super().action(gamestate)
            # END Problem 12
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        dic = {}
        distance = 0
        pos = self.place #Put second loop that goes through insects
        while pos.is_hive == False: #Go through bees list, one ant, keep track of distance
            for bee in pos.bees:
                dic[bee] = distance
            if pos.ant:
                if not (distance == 0 and pos.ant.name == 'Laser'):
                    dic[pos.ant] = distance
            pos = pos.entrance
            distance += 1
        return dic
        # END Problem Optional 2
